...
if __name__=='__main__':
    ...
    # v2_annotations_folder = 'scan2cad_annotator/test'
    if not os.path.exists(v2_annotations_folder): os.makedirs(v2_annotations_folder, exist_ok=True)

    overlap_max_thresh = 1.
    ...
    for i in range(1, len(colors)):
        color_number_str += f"{color_names[i]}-{i},"
    ...
    annotated_scenes = [os.path.basename(f)[len("s2c_v2_completion_"):-len('.json')] for f in annotated_files]
    ...
    for scene_idx, scene_id in enumerate(tqdm(scan2cad_annos.keys())):
        ...
        if not scene_id in annotated_scenes:
            ...
            out_path = os.path.join(v2_annotations_folder, "s2c_v2_completion_"+scene_id+".json")
            ...
            for idx, model in enumerate(aligned_cad_models):
                ...
                inst_overlaps = []
                max_overlap=0
                max_overlap_inst = None
                
                for idx1, inst_tuple in enumerate(valid_insts):
                    # instance = scannet_instances[inst[1]]
                    # pcd = instance['pcd']
                    inst = list(inst_tuple)
                    pcd = PCFromArray(np.array(inst[2].points), colors[idx1+1])
                    inst[2] = pcd
                    overlap = ComputePointcloudOverlap(cad_pcd, pcd, distance_threshold=0.1)
                    
                    # ShowPointCloud([model['cad'], instance['pcd']], window_name=f"Overlap: {overlap}")
                    inst_overlaps.append([overlap, inst])
                    if overlap>max_overlap:
                        max_overlap = overlap
                        max_overlap_inst = inst
                ...
                if max_overlap<=overlap_min_thresh:
                    ...
                    auto_anno_cnt+=1
                    scene_s2c_anno['aligned_models'][idx]['scannet_instance_id'] = int(max_overlap_inst[1])
                    matched_pcds.extend([model['cad'], max_overlap_inst[2]])
                    # ShowPointCloud([model['cad'], overlap_inst[2]])
                # elif scene_s2c_anno['aligned_models'][idx]['scannet_instance_id'] >= 0:
                else:
                    manual_anno_cnt+=1
                    
                    # New visualization logic: Visualize multiple instances simultaneously and select matching instance based on color
                    # scene_s2c_anno['aligned_models'][idx]['scannet_instance_id'] = -1
                    visualizer = o3d.visualization.VisualizerWithKeyCallback()
                    window_name = f"{scene_idx}/{len(scan2cad_annos.keys())} Scene: {scene_id}, obj #{idx}/{n_models},"+color_number_str
                    visualizer.create_window(window_name=window_name, width=1500, height=1400, left=1600, top=800)
                    cad_pcd = model['cad'].sample_points_uniformly(4096)
                    cad_pcd = PCFromArray(np.array(cad_pcd.points), colors[0])
                    visualizer.add_geometry(cad_pcd)
                    scene_s2c_anno['aligned_models'][idx]['scannet_instance_id'] = -1
                    
                    overlap_str = ""
                    for idx1, io in enumerate(inst_overlaps):                    
                        max_overlap, max_overlap_inst = io
                        dist, inst_id, inst_pcd = max_overlap_inst
                        visualizer.add_geometry(inst_pcd)
                        overlap_str += f"- {color_names[idx1+1]}:{max_overlap:.2f} #{len(inst_pcd.points)} "

                    selected_index = [999]
                    # Setup callbacks
                    def accept_callback_1(vis, selected_index=selected_index):
                        selected_index[0] = 0                 
                        visualizer.close()

                    def accept_callback_2(vis, selected_index=selected_index):
                        selected_index[0] = 1
                        visualizer.close()

                    def accept_callback_3(vis, selected_index=selected_index):
                        selected_index[0] = 2
                        visualizer.close()
                        
                    def accept_callback_4(vis, selected_index=selected_index):
                        selected_index[0] = 3
                        visualizer.close()
                        
                    def accept_callback_5(vis, selected_index=selected_index):
                        selected_index[0] = 4
                        visualizer.close()
                    
                    def accept_callback_6(vis, selected_index=selected_index):
                        selected_index[0] = 5
                        visualizer.close()
                        
                    def accept_callback_7(vis, selected_index=selected_index):
                        selected_index[0] = 6
                        visualizer.close()
                    ...
                    visualizer.register_key_callback(ord('1'), accept_callback_1)
                    visualizer.register_key_callback(ord('2'), accept_callback_2)
                    visualizer.register_key_callback(ord('3'), accept_callback_3)
                    visualizer.register_key_callback(ord('4'), accept_callback_4)
                    visualizer.register_key_callback(ord('5'), accept_callback_5)
                    visualizer.register_key_callback(ord('6'), accept_callback_6)
                    visualizer.register_key_callback(ord('R'), reject_callback)
                    
                    visualizer.run()
                    visualizer.destroy_window()

                    if selected_index[0]<len(inst_overlaps):
                        si = selected_index[0]
                        inst_id = inst_overlaps[si][1][1]
                        scene_s2c_anno['aligned_models'][idx]['scannet_instance_id'] = int(inst_id)
                        # print(f"Accept key pressed: {selected_index[0]}, inst#: {inst_id}")
                        matched_pcds.extend([cad_pcd, inst_overlaps[si][1][2]])
                    else:
                        print(f"*** Invalid Selection: {selected_index[0]}, #instances: {len(inst_overlaps)}")
                        
                    
                    # Old visualization logic: Visualize one instance at a time and accept instance that matches
                    # for io in inst_overlaps:
                    #     max_overlap_inst = io[1]
                    #     max_overlap = io[0]
                    #     if max_overlap>=overlap_min_thresh:
                    #         window_name = f"Scene: {scene_id}, obj #{idx}/{n_models}, Overlap: {max_overlap}, #points: {len(max_overlap_inst[2].points)}"
                    ...
                    #         visualizer.add_geometry(max_overlap_inst[2])
            ...
            window_name = f"{scene_idx}/{len(scan2cad_annos.keys())} Scene: {scene_id}, obj #{idx}/{n_models},"+color_number_str
            visualizer.create_window(window_name=window_name, width=1500, height=1400, left=1600, top=800)
            for m in matched_pcds:
                visualizer.add_geometry(m)
            visualizer.run()
            visualizer.close()
            visualizer.destroy_window()
